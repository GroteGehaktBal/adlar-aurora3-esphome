...
![Status](https://img.shields.io/badge/status-active%20sidecar%20validated%20on%20one%20JAN%20bus-2da44e)
...
> Status: the ESPHome configurations have been validated and successfully compiled for the Seeed Studio XIAO ESP32C6. Live JÅN bus traffic has been decoded with the Arduino passive analyzer at 9600 baud, 8N2, slave `1`. A slow active sidecar profile has also been validated on one real installation: it reads known registers successfully while sharing the existing JÅN RS485 bus.
...
6. If passive monitoring sees only one side of the conversation, flash `adlar_aurora3_xiao_esp32c6_active_probe.yaml`.
7. If the active probe gets replies, use `adlar_aurora3_xiao_esp32c6_active_sidecar.yaml` for the complete Home Assistant entity set.
8. Keep `Enable write controls` off until read-only values are stable and plausible.
...
The older `modbus_controller` profile is still available as `adlar_aurora3_xiao_esp32c6.yaml`, but the custom active sidecar profile is preferred for a shared JÅN bus because it uses the timing behavior proven during hardware bring-up.
...
## Active Sidecar Firmware

Use `adlar_aurora3_xiao_esp32c6_active_sidecar.yaml` after the active probe has proven that your ESP can read the heat pump. This is the recommended full-featured profile for the tested shared-bus setup.

It does not use ESPHome's standard `modbus_controller`. Instead, it implements a small custom Modbus RTU scheduler that:

- listens to the existing JÅN bus traffic;
- waits for a quiet window before transmitting;
- reads one register at a time every 2 seconds when the bus is quiet;
- maps known input and holding registers to Home Assistant sensors;
- exposes guarded write controls using function code 6 single-register writes.

Because it reads more than 60 registers one at a time, the first complete population of all entities can take a few minutes. This is intentional. A slow, boring bus is a happier bus.

Useful diagnostic entities:

- `Active sidecar read requests`
- `Active sidecar read replies`
- `Active sidecar timeouts`
- `Active sidecar bus busy skips`
- `Active sidecar last frame`
...
The active ESPHome profiles do act as an additional Modbus client/master. RS485 supports multiple electrical nodes on the same pair, but Modbus RTU has no real bus arbitration. Public Aurora III community setups work by tapping the same JÅN/internal bus and polling gently.

The active sidecar profile therefore sends only after a quiet bus window and handles one register per request. Do not make it faster until the bus has been proven stable over several hours.
...
- Live telemetry uses input registers (Modbus function code 4).
